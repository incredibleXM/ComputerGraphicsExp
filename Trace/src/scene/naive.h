...
public:
	...
	KDTNode *pChild[2];			// pointers to any children nodes
	unsigned short subdiv_axis;			// which axis does the subdivision plane lie on?

public:
	KDTNode(const BoundingBox& bounds, const std::vector<Geometry*> &parentMembers, short subdiv_axis)
	{
		this->bounds = bounds;
		members.clear();
...
public:
	...
	void Subdivide(unsigned short depth, unsigned short maxDepth, unsigned long maxListLength)
	{
		...
		if ((depth < maxDepth) && (members.size() > maxListLength)) {
			...
			pChild[0] = new KDTNode(childBounds, members, newAxis);
			...
			pChild[1] = new KDTNode(childBounds, members, newAxis);
...
{
private:
	BoundingBox bounds;				// bounds of the entire KD tree
	...
	KDTNode *pRoot;					// Pointer to root node
...
public:
	...
	void initialize(std::vector<Geometry*> *pObjectList, unsigned short maxTreeDepth, unsigned long maxChildrenPerNode, const BoundingBox& sceneBounds)
	{
		...
		pRoot = new KDTNode(bounds, *pObjects, 0);
