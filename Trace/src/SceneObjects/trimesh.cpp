...
bool TrimeshFace::intersectLocal( const ray& r, isect& i ) const
{
	...
	double normalDotDirection = normal * d;
	if(normalDotDirection != 0) {
		double t = (facePlaneD - (normal * p)) / normalDotDirection;
		if(t > RAY_EPSILON) {
			Vec3d q = r.at(t);
			double abqDotN = ((b-a) ^ (q-a)) * normal;
			double qbcDotN = ((c-b) ^ (q-b)) * normal;
			double aqcDotN = ((a-c) ^ (q-c)) * normal;
			if(abqDotN >= 0 && qbcDotN >= 0 && aqcDotN >= 0) {
				if(parent->normals.size()>0) {
					double alpha = qbcDotN / barycentricDenominator;
					double beta = aqcDotN / barycentricDenominator;
					double gamma = abqDotN  / barycentricDenominator;
